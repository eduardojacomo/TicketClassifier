...
defineProps({
  ...
  existingBatch: { type: Object, required: true }
})
const emit = defineEmits(['overwrite', 'create', 'cancel'])
</script>

<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" @click.self="emit('cancel')">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full">
      ...
      <div class="p-6 space-y-4">
        <p class="text-sm text-slate-600">
          A processed batch already exists with the file <b class="text-slate-900">{{ fileName }}</b>.
        </p>
        ...
        <div class="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-2">
          <div class="flex items-center justify-between text-sm">
            ...
            <span class="font-bold text-slate-900">{{ existingBatch.total }}</span>
          </div>
          <div class="flex items-center justify-between text-sm">
            <span class="text-slate-500">Processed on</span>
            <span class="font-medium text-slate-700">{{ formatDate(existingBatch.createdDate) }}</span>
          </div>
        </div>
